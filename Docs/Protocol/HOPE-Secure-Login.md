...
| 0x0EC2 | Client Cipher | Algorithm list (same encoding) | *Optional.* Ciphers the client supports |
| 0x0EC1 | Server Cipher | Algorithm list | *Optional.* Ciphers requested for server → client |
| 0x0ECA | Client Compression | Algorithm list | *Optional.* Compression algorithms supported |
| 0x0EC9 | Server Compression | Algorithm list | *Optional.* Compression algorithms requested |
...
The server responds with a `htlsHdrTask` (65536) reply:
...
|---|---|---|
| Server IP | 4 bytes | IPv4 address of the server (big-endian) |
| Server port | 2 bytes | Port number (big-endian) |
| Random bytes | 58 bytes | Cryptographically random |

**Total length:** 64 bytes.

The client can compare the IP portion against the address it connected to as a basic authenticity check. If the values do not match, the client should warn the user.
...
GLoarbLine, for example, does not send `DATA_HOPE_APP_ID` in its HOPE identification; it only includes the MAC algorithm and cipher fields.
